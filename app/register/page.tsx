...
function RegisterForm() {
  ...
  return (
    <AuthShell
      ...
      title={
        <>
          The audit <em className="not-italic text-gradient-violet">starts</em>
          {" "}here.
        </>
      }
      subtitle="Free forever for the first five subscriptions. No card. Takes ninety seconds. We'll do the worrying for you."
    >
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Free forever for up to five subscriptions.
      </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <ul className="space-y-1 text-xs text-muted-foreground">
            <li className="flex items-center gap-1.5">
              <Check
                className={`h-3 w-3 ${
                  password.length >= 8 ? "text-emerald-400" : "text-muted-foreground/40"
                }`}
              />
          ...
          >
            {loading ? "Creating account…" : "Create my audit"}
